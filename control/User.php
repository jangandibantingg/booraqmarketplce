<?php
    ...
    class User
    {
        ...
        public function register($email, $password, $nama_depan, $nama_belakang, $no_hp, $kota)
        {
            try
            {
        ...
        public function activation($email, $password, $nama_depan, $nama_belakang, $no_hp, $kota)
        {
            try
            {
              $hashPasswd = md5($password);
              $query = $this->db->prepare("INSERT into user (email,password,nama_depan,nama_belakang,no_hp,id_kota)
              values (:email, :password, :nama_depan,:nama_belakang, :no_hp, :kota) ");
              $query->bindParam(":email", $email);
            ...
            try
            {
                ...
                if($query->rowCount() > 0){
                    ...
                    if(md5($password) == $data['password']){
                        ...
                        if ($data['level'] == 0) {
                          $this->true = "Login Success Redirect to dashboarrd <script type='text/javascript'> window.location.href = './profile.aspx' </script>";
                        }else {
                          $this->true = "Login Success Redirect to dashboarrd <script type='text/javascript'> window.location.href = './admin.aspx' </script>";
        ...
        public function gettoko($username){
          try {
            $query = $this->db->prepare("SELECT * FROM toko WHERE email = :email");
            $query->bindParam(":email", $username);
            $query->execute();
            return $query->fetch();
          } catch (Exception $e) {
